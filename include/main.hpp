...
public:
	...
	auto onNewID(std::function<void(int)> callback, bool prepend = false, bool replace = false) {
		if (this->getUserObject("m_onNewID was reserved")) return this;
		if (replace) {
			this->setUserObject("m_onNewID was reserved", GameManager::get()); // clear();
			m_onNewID.clear();
		}
		prepend ? (void)m_onNewID.insert(m_onNewID.begin(), callback) : m_onNewID.push_back(callback);
		return this;
	}
	// Called when the input is created and got basic setup
	auto onInputSetup(std::function<void(geode::TextInput*)> callback, bool prepend = false, bool replace = false) {
		...
		prepend ? (void)m_onInputSetup.insert(m_onInputSetup.begin(), callback) : m_onInputSetup.push_back(callback);
		return this;
	}
	// Called when the label is created and got basic setup
	auto onLabelSetup(std::function<void(cocos2d::CCLabelBMFont*)> callback, bool prepend = false, bool replace = false) {
		...
		prepend ? (void)m_onLabelSetup.insert(m_onLabelSetup.begin(), callback) : m_onLabelSetup.push_back(callback);
		return this;
	}
	// Called when the input text is changed
	auto onInputTextChanged(std::function<void(const std::string&)> callback, bool prepend = false, bool replace = false) {
		...
		prepend ? (void)m_onInputTextChanged.insert(m_onInputTextChanged.begin(), callback) : m_onInputTextChanged.push_back(callback);
		return this;
	}
	// Called when the account login layer is shown and not yet setuped yet
	auto onAccountLoginLayerShow(std::function<void(AccountLoginLayer*)> callback, bool prepend = false, bool replace = false) {
		...
		prepend ? (void)m_onAccountLoginLayerShow.insert(m_onAccountLoginLayerShow.begin(), callback) : m_onAccountLoginLayerShow.push_back(callback);
		return this;
	}
	// Called when the account login layer is shown and setuped
	auto onAccountLoginLayerSetup(std::function<void(AccountLoginLayer*)> callback, bool prepend = false, bool replace = false) {
		...
		prepend ? (void)m_onAccountLoginLayerSetup.insert(m_onAccountLoginLayerSetup.begin(), callback) : m_onAccountLoginLayerSetup.push_back(callback);
		return this;
	}
};
